...
STRATEGIES = [COOPERATE, DEFECT, POOLOSTRACIZE, PEEROSTRACIZE]#, OSTRACIZED]
COLORS = [COLOR_X, COLOR_Y, COLOR_R, COLOR_S]#, COLOR_O]
...
def determineAvailableStrategies(case, participation = COMPULSORY):
    ...
    if case in [POOL_OSTRACISM, COMPETITION]:
        available_strategies.append(POOLOSTRACIZE)
    if case in [PEER_OSTRACISM, COMPETITION]:
        available_strategies.append(PEEROSTRACIZE)
    #available_strategies.append(OSTRACIZED)
...
def plotCase(case, participation, position = 121, param = "s", order="", pref="", solo=False):
...
def plotGraphs(case, param="s", order="", solo = False, participation = COMPULSORY):
    ...
    if not solo:
        ax = plotCase(case, VOLUNTARY, 121, param, order)
        ax = plotCase(case, VOLUNTARY, 122, param, order, pref="hurt")
    else:
...
if __name__ == "__main__":
    ...
    plotGraphs(PEER_OSTRACISM, "s", SECOND_ORDER, solo = False)#, participation = VOLUNTARY)
