...
class SocialLearningSim:
    ...
    def createDataFile(self, case, factor):
        ...
        data_file = open("data2/{}/{}.csv".format(factor, name), "w")
    ...
    def probaNoT(self):
        ...
        return comb(M - T - 2, N - 2) / comb(M - 2, N - 2)
    ...
    def getPayoffXrj(self):
        ...
        return payoff + SOP_pool_restored + SOP_peer_restored
    ...
    def getPayoffT(self):
        ...
        if self.SOPallowed():
            ...
            redistributed_restoration = (restored_welfare / N) * avg_X_in_sample
    ...
    def getPayoffU(self):
        ...
        Y_rj, Z = self.getFrequency(DEFECT_RJ), self.getFrequency(ABSTAIN)
        ...
        avg_fine = gamma * sample_proba * Y_rj
        ...
        return payoff - avg_fine + restored_welfare - redistributed_restoration + satisfaction
    ...
    def getPayoffW(self):
        ...
        if self.SOPallowed():
            X_cj = self.getFrequency(COOPERATE_CJ)

            avg_fee = (((N - 1) * X_cj) / (M - 1)) * gamma * (1 - self.probaNoYcj())
            if self.getCase() == COMPETITION:
                pool_setback = self.poolSetback()

        payoff = self.getRawPayoffX() - ((((N - 1) * Y_cj)/(M - 1)) * gamma)
    ...
    def saveResults(self, res_vector, name, factor="mu"):
        ...
        file_name = "RJ/{}{}_{}.csv".format(punish_flag, name, case)
...
if __name__ == "__main__":
    SLS = SocialLearningSim(N = 5,                      # only extreme case of N = M
                            M = 9,                    # size of M seems to impact levels
